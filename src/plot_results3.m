function [ospa_vals, ospa2_cell, handles] = plot_results3(model,truth,meas,est,tbirth)
...
%plot ground truths
figure('visible','on'); truths= gcf; hold on;
...
figure('visible','on'); tracking= gcf; hold on;
...
for j=1:length(meas)
    for k=1:meas{j}.K
        if ~isempty(meas{j}.Z{k})
            hlined= line(k*ones(size(meas{j}.Z{k},2),1),meas{j}.Z{k}(1,:),'LineStyle','none','Marker','x','Markersize',5,'Color',0.7*ones(1,3)); %.*sin(meas{j}.Z{k}(1,:)
        end
    end
end
...
for j=1:length(meas)
    for k=1:meas{j}.K
        if ~isempty(meas{j}.Z{k})
            yhlined= line(k*ones(size(meas{j}.Z{k},2),1),meas{j}.Z{k}(2,:),'LineStyle','none','Marker','x','Markersize',5,'Color',0.7*ones(1,3)); %.*cos(meas{j}.Z{k}(1,:))
        end
    end
end
...
subplot(211); xlabel('Time','interpreter','latex'); ylabel('x-coordinate (m)','interpreter','latex');
set(gca, 'XLim',[1 truth.K]); set(gca, 'YLim',[-model.range_c(2,2) model.range_c(2,2)],'TickLabelInterpreter','latex');
legend([yhline2 yhline1 hlined],'Estimates          ','True tracks','Measurements');

subplot(212); xlabel('Time','interpreter','latex'); ylabel('y-coordinate (m)','interpreter','latex');
set(gca, 'XLim',[1 truth.K]); set(gca, 'YLim',[ -model.range_c(2,2) model.range_c(2,2)] ,'TickLabelInterpreter','latex'); %'YLim',[ model.range_c(1,2) model.range_c(2,2)]
...
figure('visible','on'); ospa= gcf; hold on;
subplot(3,1,1); plot(1:meas{1}.K,ospa_vals(:,1),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist');
subplot(3,1,2); plot(1:meas{1}.K,ospa_vals(:,2),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc');
subplot(3,1,3); plot(1:meas{1}.K,ospa_vals(:,3),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card');
xlabel('Time');
...
win_len= 10;
 ...
 figure('visible','on'); ospa2= gcf; hold on;
 windowlengthlabels = cell(1,length(win_len));
 subplot(3,1,1);
 for i = 1:length(win_len)
     plot(1:truth.K,ospa2_cell{i}(1,:),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Dist','interpreter','latex');
 ...
 subplot(3,1,2);
 for i = 1:length(win_len)
     plot(1:truth.K,ospa2_cell{i}(2,:),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Loc','interpreter','latex');
     windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
 end
 
 subplot(3,1,3);
 for i = 1:length(win_len)
     plot(1:truth.K,ospa2_cell{i}(3,:),'k'); grid on; set(gca, 'XLim',[1 meas{1}.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Card','interpreter','latex');
     windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
 end
