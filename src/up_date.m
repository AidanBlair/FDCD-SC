function glmb_update= up_date(glmb_predict,model,filter,meas,k,SenNum)
%create updated tracks (single target Bayes update)
try
   m= size(meas.Z{k},2);
   disp(m)
catch
    disp('Ooops in up_date')
end
...
for emm= 1:m
    for tabidx= 1:length(glmb_predict.tt)
        ...
        try
            w_temp= compute_pD(model,glmb_predict.tt{tabidx}.x,SenNum,0).*glmb_predict.tt{tabidx}.w(:).* ...
            compute_likelihood_original(model,meas.Z{k,1}(:,emm),glmb_predict.tt{tabidx}.x,SenNum)'; 
        catch
            disp('wtemp error')
        end
...
for tabidx=1:length(glmb_predict.tt)
    avpd(tabidx)= glmb_predict.tt{tabidx}.w(:)'*compute_pD(model,glmb_predict.tt{tabidx}.x,SenNum,0)+eps(0);
end
avqd= 1-avpd; %1-avpd;
