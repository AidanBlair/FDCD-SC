function tt_lmb_merged=EuclidianMerge2(tt_lmb,filter,Scenario)
...
if Scenario == 1        % Scenario 1
    dthresh = 5;
elseif Scenario == 2    % Scenario 2
    dthresh = 0.01;
end
%dthresh=0.01; %euclidiean distance threshold for state of n dimensions (here is 2)
%dthresh=5;
...
%         X{i}
        ...
        %Xmean(:,i)=sum(bsxfun(@times,w{i}',X{i}([1,3],:)),2);
    ...
    cm= ((distmatrixm) <= dthresh); %% logic matrix where values of 1 correspond to values less than the threshold 
...
for i=1:L %%put merged components back into a lmb
